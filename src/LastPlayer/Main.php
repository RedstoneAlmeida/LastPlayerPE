...
class Main extends PluginBase implements Listener
{
	...
	*/
	...
	public function onEnable()
	{
		...
		$this->all=0;//最大玩家数量
		$this->config->save();
		$this->getServer()->getLogger()->info(TextFormat::GREEN."[SG] Plugin loaded!");
		//$this->getServer()->getLogger()->info(TextFormat::GREEN."[FSurvivalGame] 如果觉得好用可以给支付宝Redtone@qq.com汇款任意数额来支持作者哦");
	...
	public function onCommand(CommandSender $sender, Command $command, $label, array $args)
	{
		if($command->getName()=="lobby")
		{
			...
			{
				$sender->sendMessage("§b[LP] §aThe game was started,you can't back to lobby.");
				return;
			}
			...
			{	
				...
				$sender->sendMessage("§b[LP]§a Back to lobby...");
				$this->sendToAll("§b[LastPlayer] §f Player ".$sender->getName()." exit from game");
				$this->changeStatusSign();
				if($this->gameStatus==1 && count($this->players)<2)
				{
					$this->gameStatus=0;
					$this->lastTime=0;
					$this->sendToAll("[LastPlayer] Less than 2 players,stopped countdown");
			...
			{
				$sender->sendMessage("§b[SG]§a You are not in the game.");
			}
			return true;
		}
		...
		case "set":
			if($this->config->exists("lastpos"))
			{
				$sender->sendMessage("§b[SG]§a The game was set before,please use /fsg remove and try again.");
			}
			else
			{
				$name=$sender->getName();
		...
		case "remove":
			...
			break;
		case "start":
			$this->sendToAll("[LastPlayer] §b§lForce start game...");
			$this->gameStatus=1;
			$this->lastTime=5;
			break;
		case "reload":
	...
	public function onPlayerCommand(PlayerCommandPreprocessEvent $event)
	{
		...
		default:
			...
			$event->getPlayer()->sendMessage("[LP] You are now in game,cannot use other commands");
			$event->getPlayer()->sendMessage("[LP] You can also use /kill or /lobby to exit game");
			break;
		}
		unset($event);
	}
	...
	public function onDamage(EntityDamageEvent $event)
	{
		...
		if ($event instanceof EntityDamageByEntityEvent)
		{
			...
			if($player instanceof Player && $killer instanceof Player)
			{
		    	...
		    	{
		    		$event->setCancelled();
		    		$killer->sendMessage("[LP] PLEASE DO NOT INTERFERENCE THE GAME");
		    		$killer->kill();
		    	}
		    }
		}
	...
	public function PlayerDeath(PlayerDeathEvent $event){
		if($this->gameStatus==3 || $this->gameStatus==4)
		{
			if(isset($this->players[$event->getEntity()->getName()]))
			{
				...
				{
					$this->sendToAll("[LastPlayer] Player {$event->getEntity()->getName()} die");
					$this->sendToAll("[LP] Players :".count($this->players));
					$this->sendToAll("[LP] Last time :".$this->lastTime." sec");
				}
				$this->changeStatusSign();
			}
	...
	public function gameTimber(){
		...
		if($this->gameStatus==1)
		{
			...
			//case 20:
			case 30:
				$this->sendToAll("§a[LastPlayer]§e The game will start in ".$this->lastTime." seconds");
				break;
			case 60:
				$this->sendToAll("§a [LastPlayer]§e The game will start in one minute");
				break;
			case 90:
				$this->sendToAll("[LastPlayer] The game will start in one minute thirty seconds");
				break;
			case 120:
				$this->sendToAll("§a[LastPlayer]§e The game will start in two minutes");
				break;
			case 150:
				$this->sendToAll("§a[LastPlayer]§e The game will start in two minutes thirty seconds");
				break;
			case 0:
				$this->gameStatus=2;
				$this->sendToAll("§a[LastPlayer] §eThe game begins");
		...
		if($this->gameStatus==2)
		{
			...
			{
				$this->gameStatus=3;
				$this->sendToAll("§a[LastPlayer] §ePvP is ENABLED!");
		...
		if($this->gameStatus==3 || $this->gameStatus==4)
		{
			if(count($this->players)==1)
			{
				$this->sendToAll(" §a[LastPlayer] §bCongratulations! You have won the game");
				foreach($this->players as &$pl)
				{
					$p=$this->getServer()->getPlayer($pl["id"]);
					Server::getInstance()->broadcastMessage("[LastPlayer] Congratulates to ".$p->getName()." for whom that has won the game");
					$p->setLevel($this->signlevel);
			...
			{
				Server::getInstance()->broadcastMessage("[LastPlayer] The game ends for all players have been dead");
				$this->gameStatus=0;
				$this->lastTime=0;
				$this->clearChest();
				$this->ClearAllInv();
		...
		if($this->gameStatus==4)
		{
			...
			//case 20:
			case 30:
				$this->sendToAll("[LP] there are ".$this->lastTime."seconds to the end of the game");
				break;
			case 0:
				$this->sendToAll("[LP] time out,the game ends");
	...
	public function changeStatusSign()
	{
		...
		if($sign instanceof Sign)
		{
			...
			{
			case 0:
				$sign->setText("§fLP","§aTap to join","§bplayer:".count($this->players),"");
				break;
			case 1:
				$sign->setText("§fLP","§aTap to join","§bplayer:".count($this->players),"§aTime:".$this->lastTime."sec");
				break;
			case 2:
				$sign->setText("§fLP","§astarting","§bplayer:".count($this->players),"§fyou are prohibited :".$this->lastTime."sec");
				break;
			case 3:
	...
	public function playerBlockTouch(PlayerInteractEvent $event){
		...
		if(isset($this->SetStatus[$username]))
		{
			...
			case 9:
				...
				$this->config->save();
				$this->SetStatus[$username]++;
				$player->sendMessage(TextFormat::GREEN."[Hunger Game] spawnpoint 9 created");
				$player->sendMessage(TextFormat::GREEN."[Hunger Game] Please click to choose a destination for the death match");
				$this->pos9=new Vector3($this->pos9["x"]+0.5,$this->pos9["y"],$this->pos9["z"]+0.5);
				break;
			case 10:
				...
				$this->config->save();
				$this->SetStatus[$username]++;
				$player->sendMessage(TextFormat::GREEN."[Hunger Game] spawnpoint 10 created");
				$player->sendMessage(TextFormat::GREEN."[Hunger Game] Please click to choose a destination for the death match");
				$this->pos10=new Vector3($this->pos10["x"]+0.5,$this->pos10["y"],$this->pos10["z"]+0.5);
				break;
			case 11:
				...
				$this->config->save();
				$this->SetStatus[$username]++;
				$player->sendMessage(TextFormat::GREEN."[Hunger Game] spawnpoint 11 created");
				$player->sendMessage(TextFormat::GREEN."[Hunger Game] Please click to choose a destination for the death match");
				$this->pos11=new Vector3($this->pos11["x"]+0.5,$this->pos11["y"],$this->pos11["z"]+0.5);
				break;
			case 12:
				...
				$this->config->save();
				$this->SetStatus[$username]++;
				$player->sendMessage(TextFormat::GREEN."[Hunger Game] spawnpoint 12 created");
				$player->sendMessage(TextFormat::GREEN."[Hunger Game] Please click to choose a destination for the death match");
				$this->pos12=new Vector3($this->pos12["x"]+0.5,$this->pos12["y"],$this->pos12["z"]+0.5);
				break;
			case 13:
				...
				$this->config->save();
				$this->SetStatus[$username]++;
				$player->sendMessage(TextFormat::GREEN."[Hunger Game] spawnpoint 13 created");
				$player->sendMessage(TextFormat::GREEN."[Hunger Game] Please click to choose a destination for the death match");
				$this->pos13=new Vector3($this->pos13["x"]+0.5,$this->pos13["y"],$this->pos13["z"]+0.5);
				break;
			case 14:
				...
				$this->config->save();
				$this->SetStatus[$username]++;
				$player->sendMessage(TextFormat::GREEN."[Hunger Game] spawnpoint 14 created");
				$player->sendMessage(TextFormat::GREEN."[Hunger Game] Please click to choose a destination for the death match");
				$this->pos14=new Vector3($this->pos14["x"]+0.5,$this->pos14["y"],$this->pos14["z"]+0.5);
				break;
			case 15:
				...
				$player->sendMessage(TextFormat::GREEN."[LastPlayer] All settings completed and you can start a game now");
				$this->level=$this->getServer()->getLevelByName($this->config->get("pos1")["level"]);					
			}
		}
		else
		{
			...
			if(isset($this->lastpos) && $this->lastpos!=array() && $sign instanceof Sign && $sign->getX()==$this->sign->x && $sign->getY()==$this->sign->y && $sign->getZ()==$this->sign->z && $event->getPlayer()->getLevel()->getFolderName()==$this->config->get("sign")["level"])
			{
				if(!$this->config->exists("lastpos"))
				{
					$event->getPlayer()->sendMessage("[LastPlayer] You can not join the game for the game hasn't been set yet");
					return;
				}
				if(!$event->getPlayer()->hasPermission("sghybrid.touch.startgame"))
				{
				...
				if(!$event->getPlayer()->isOp())
				{
					...
					for($i=0;$i<$inv->getSize();$i++)
    				{
    					...
    					{
    						$event->getPlayer()->sendMessage("[LastPlayer] Before the game,please put things in your bag to your case");
    						return;
    					}
    				}
    				foreach($inv->getArmorContents() as $i)
    				{
    					if($i->getID()!=0)
    					{
    						$event->getPlayer()->sendMessage("[LastPlayer] Please take off your equipments and put them in the case");
    						return;
    					}
    				}
    			}
				if($this->gameStatus==0 || $this->gameStatus==1)
				{
					if(!isset($this->players[$event->getPlayer()->getName()]))
					{
						...
						{
							$event->getPlayer()->sendMessage("[LastPlayer] the map is full,no spare place for more");
							return;
						}
						$this->sendToAll("§b[LastPlayer]Player ".$event->getPlayer()->getName()." joined the game");
						$this->players[$event->getPlayer()->getName()]=array("id"=>$event->getPlayer()->getName());
						$event->getPlayer()->sendMessage("[LastPlayer] joined the game successfully");
						if($this->gameStatus==0 && count($this->players)>=2)
						{
							$this->gameStatus=1;
							$this->lastTime=$this->waitTime;
							$this->sendToAll("[LastPlayer] The game will countdown when reach the lowest people amount level");
						}
						if(count($this->players)==8 && $this->gameStatus==1 && $this->lastTime>5)
						{
							$this->sendToAll("[LastPlayer] Already full,starting");
							$this->lastTime=5;
						}
						$this->changeStatusSign();
					}
					...
					{
						$event->getPlayer()->sendMessage("[LastPlayer] You are already in,input/lobby   may exit from the game");
					}
				}
				else
				{
					$event->getPlayer()->sendMessage("[LastPlayer] Can not join the game for it has already started");
				}
			}
		}
	}
	...
	public function PlayerQuit(PlayerQuitEvent $event){
		...
		{	
			...
			$this->sendToAll("[LastPlayer] Player".$event->getPlayer()->getName()."has left the game");
			$this->changeStatusSign();
			if($this->gameStatus==1 && count($this->players)<2)
			{
				$this->gameStatus=0;
				$this->lastTime=0;
				$this->sendToAll("[LastPlayer] no enough players,stop counting down");
				/*foreach($this->players as $pl)
				{
					$p=$this->getServer()->getPlayer($pl["id"]);
					$p->setLevel($this->signlevel);
